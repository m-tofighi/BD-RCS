function [mval,g]=subproblem_cost_V0(x,C1,C2,C1T,C2T,maxrank,meas_fft,y,sigma,siglen,n1,n2,lambda,w)
    ...
    mval = norm(Z,'fro')^2+norm(H,'fro')^2 - 2*real(y'*dev) + sigma*norm(dev,'fro')^2 + lambda^2*sum(sum((Z*H').^2, 2)./w); 
    ...
    w_inv = 1./w;
    GradZ = 2*(Z - adjoint_times_H + lambda^2*bsxfun(@times, w_inv, Z*H'*H));
    GradH = 2*(H - adjoint_times_Z + lambda^2*H*Z'*bsxfun(@times, w_inv, Z));
