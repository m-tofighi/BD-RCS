function [Z,H] = blindDeconvolve_implicit_2D(conv_zh,C1,C2,maxrank,C1T,C2T,ZInit,HInit,lambda, maxOutIter,Csize,sparsity)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %  Solving problem 5 in paper
    %  function [Z,H] = blindDeconvolve(conv_zh,C1,C2,maxrank,C1T,C2T,ZInit,HInit,lambda, maxOutIter)
    %
    ...
    vec = @(x) x(:);
    n1 = length(C1T(mat(conv_zh))); %size(ZInit,1);%length(C1T(mat(conv_zh))); % There must be a better way to do this...
        n2 = length(C2T(mat(conv_zh)));%size(HInit,1);%length(C2T(mat(conv_zh)));%
    ...
    if sparsity == 1
        w = lambda*sqrt(sum((H*Z').^2,2)) + epsilon;
    elseif sparsity == 2
        w = lambda*sqrt(sum((Z*H').^2,2)) + epsilon;
    end
    ...
    for outIter=1:pars.maxOutIter,
        ...
        if sparsity == 1
            [x,mval] = minFunc(@subproblem_cost,[Z(:);H(:)],options,C1,C2,C1T,C2T,maxrank,meas_fft,y,sigma,siglen,n1,n2,lambda,w);
        elseif sparsity == 2
            [x,mval] = minFunc(@subproblem_cost_V0,[Z(:);H(:)],options,C1,C2,C1T,C2T,maxrank,meas_fft,y,sigma,siglen,n1,n2,lambda,w);
        end
        
        Z = reshape(x(1:(n1*maxrank)),n1,maxrank);
        H = reshape(x((n1*maxrank) + (1:(n2*maxrank))),n2,maxrank);
        if sparsity == 1
            w = lambda*sqrt(sum((H*Z').^2,2)) + epsilon/(ik)^2;
            ik=ik+1;
        elseif sparsity == 2
            w = lambda*sqrt(sum((Z*H').^2,2)) + epsilon/(ik)^2;
            ik=ik+1;
        end
        ...
        %imagesc(abs(H));
        %colorbar;
        %title(sprintf('lambda=%f,iteration=%d\n', lambda,outIter));
        %pause(0.001);
    end
